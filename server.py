"""
VERNEVAKT BACKEND - HMS-dokumentasjon for norske byggeplasser
================================================================
Vernevakt - Sikker HMS-dokumentasjon
...
CONFIG = {
    ...
    'signing_key': os.environ.get('SIGNING_KEY', 'vernevakt-default-key-change-in-production'),
}
...
def generate_sha_report(data, photos, output_path):
    ...
    if report_type == 'fare':
        ...
        story.append(Paragraph(f"VERNEVAKT RAPPORT: {template['name'].upper()}", title_style))
    ...
    # ===== DEVIATION DETAILS TABLE (only if deviations exist) =====
    failed_items = [(item_id, dev) for item_id, dev in deviations.items()
                    if checklist.get(item_id) == False]

    if failed_items:
        story.append(Paragraph("AVVIK — ANSVARLIG OG FRIST", header_style))
        story.append(Spacer(1, 3*mm))

        dev_data = [['Sjekkpunkt', 'Ansvarlig', 'Frist for utbedring']]
        for item_id, dev in failed_items:
            # Find item text from template
            item_text = dev.get('item_text', item_id)
            ansvarlig = dev.get('ansvarlig', '—') or '—'
            frist_raw = dev.get('frist', '')
            if frist_raw:
                try:
                    from datetime import datetime as dt
                    frist = dt.strptime(frist_raw, '%Y-%m-%d').strftime('%d.%m.%Y')
                except:
                    frist = frist_raw
            else:
                frist = '— Ikke angitt'
            dev_data.append([item_text, ansvarlig, frist])

        dev_table = Table(dev_data, colWidths=[85*mm, 45*mm, 35*mm])
    ...
    ─────────────────────────────────────────────────────────────────────<br/>
    Dette dokumentet er generert automatisk av Vernevakt.<br/>
    ...
    Dokumentet skal oppbevares i minimum 5 år etter prosjektets avslutning.<br/>
    <br/>
    <b>Ansvarsfraskrivelse:</b> Dette dokumentet støtter HMS-dokumentasjon på byggeplassen.
    Ansvaret for sikkerhet ligger hos arbeidsgiver, entreprenør og byggherre
    i henhold til gjeldende lovgivning. Vernevakt fraskriver seg ikke dette ansvaret.
    </font>
    """
...
Vernevakt - Automatisk generert faremelding
"""
...
# ============================================
class VernevaktHandler(BaseHTTPRequestHandler):
    ...
    def _handle_submit(self, data):
        ...
        try:
            ...
            pdf_filename = f"vernevakt_rapport_{timestamp}.pdf"
